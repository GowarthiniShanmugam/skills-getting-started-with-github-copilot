        ...
        if (details.participants && details.participants.length > 0) {
          participantsHTML = `<ul class="participants-list">${details.participants.map(p => `
            <li class="participant-item">
              ...
              <button class="delete-participant-btn" title="Remove participant" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(p)}">✖</button>
            </li>`).join("")}</ul>`;
        } else {
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        signupForm.reset();
        fetchActivities(); // Refresh activity cards after signup
      } else {
